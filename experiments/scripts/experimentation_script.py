...
execution_params = {
    "experiment_root_path" : "/export/share/peters57dm/Verbund/deepsync/experiments/comparison301",
    ...
    "gif_duration" : 300,
    "pretrained_model_path" : "/export/share/peters57dm/Verbund/data/version-2_mse_pretrained_models256-128-64",
...
save_dict_as_json(execution_params,
            os.path.join(base_path, "experiment_settings.json"))

#Execution
n_experiments = len(loss_fns) * len(labels_assignment_methods) * len(datasets_loading_methods) * experiment_repetitions
exp_follower = 1
    ...
    for lam in labels_assignment_methods:
        ...
        for ds_loader in datasets_loading_methods:
            ...
            for exp_i in range(experiment_repetitions):
                experiment_directory_path = os.path.join(dataset_experiment_path, 'exp_{0:02}'.format(exp_i))
                ...
                if do_deepsync_train:
                    ...
                    model, labels_over_iterations = deep_sync_model_ship(
                        device, model, data, data_name,
