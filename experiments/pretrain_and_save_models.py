...
N_MODELS = 5
pretrained_models_path = f"/export/share/peters57dm/Verbund/data/n-pretrained-models/mse_pretrained_models{ae_layers[0]}-{ae_layers[1]}-{ae_layers[2]}"
...
for loading_method in tqdm(datasets_loading_methods, total=len(datasets_loading_methods)):
    for i in range(N_MODELS):
        ...
        _pretrained_model_path = os.path.join(pretrained_models_path, f"pretrained_{data_name}_{i}.pth")
        model.fit(
            n_epochs=n_epochs,
            ssl_loss_fn=ssl_loss_fn,
            dataloader=trainloader,
            model_path=_pretrained_model_path,
            optimizer_params={"lr": lr},
            optimizer_class=torch.optim.Adam,
        )
        mse_loss = model.evaluate(trainloader, ssl_loss_fn, device)
        _res = {f"pretrained_{data_name}_{i}": float(mse_loss)}
        save_dict_as_json(_res, os.path.join(pretrained_models_path, f"pretrained_{data_name}_{i}_loss.json"))
